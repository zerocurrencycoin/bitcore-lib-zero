...
Transaction.prototype.fromBuffer = function(buffer) {
  ...
  try {
    ...
    // A malformed, truncated, or otherwise unexpected tx buffer can make the
    // reader run off the end of the data, so BufferReader.readUInt32LE throws
    // "RangeError: Index out of range" (bufferreader.js). This has been seen
    // on tx buffers delivered over the ZMQ rawtx topic. Re-throw with context
    // (buffer length + parsed version) so callers and logs can identify it
    // instead of seeing a bare RangeError. Callers that receive untrusted tx
    // buffers (e.g. the ZMQ inv handler) MUST wrap this in try/catch so one
    // unparseable tx cannot crash the whole process.
    var v = (typeof this.version !== 'undefined') ? this.version : 'unknown';
    throw new Error('Transaction.fromBuffer: failed to parse tx (parsed version ' +
      v + ', buffer length ' + buffer.length + ' bytes): ' + err.message);
  }
};
